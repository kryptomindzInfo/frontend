...
closePopup = () =>{
  this.setState({popup: false, name: '', address1: '', address2: '', email: '', mobile: '', logo: null, contract: null });
}
...
addBank = (event) => {
  event.preventDefault();
  axios.post(API_URL+'/addBank', {name: this.state.name, address1: this.state.address1, address2 : this.state.address2, email: this.state.email, mobile: this.state.mobile, logo: this.state.logo, contract: this.state.contract})
  .then(res => {
    if(res.status == 200){
      if(res.data.error){
...
fileUpload(file, key){
  ...
  axios.post(API_URL+'/fileUpload', formData, config)
  .then(res => {
    if(res.status == 200){
      if(res.data.error){
...
getBanks = () => {
  axios.get(API_URL+'/getBanks', {token: token})
    .then(res => {
       if(res.status == 200){
