...
export default class EditFee extends Component {
  ...
  addRange = () => {
    const temp = this.state.ranges;
    const l = temp.length;
    let last = temp[l - 1].trans_to;
    if (last == '') {
      this.setState(
        {
  ...
  getFees = () => {
    axios
      .post(`${API_URL}/getRule`, { token, rule_id: this.state.rule_id })
      .then(res => {
        if (res.status == 200) {
          const temp = JSON.parse(res.data.rules.ranges);
          this.setState(
            {
              loading: false,
              name: res.data.rules.name,
              trans_type: res.data.rules.trans_type,
              active: res.data.rules.active,
              ranges: temp,
              loading: false,
            },
            () => {
            ...
            <Card bigPadding centerSmall>
              <div className="cardHeader">
                <div className="cardHeaderLeft flex">
                  ...
                  <h3>Edit Revenue sharing Rule</h3>
                </div>
              </div>
