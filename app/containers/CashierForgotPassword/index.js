...
export default class CashierForgotPassword extends Component {
  ...
  forgotRequest = event => {
    ...
    axios
      .post(`${API_URL}/cashierForgotPassword`, this.state)
      .then(res => {
        if (res.status == 200) {
          localStorage.setItem('cashierPhone', res.data.mobile);
          localStorage.setItem('cashierUserName', res.data.username);
          
          history.push('/cashier/'+this.props.match.params.bank+'/otp');
