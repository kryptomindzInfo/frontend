...
export default class BankBranchList extends Component {
  ...
  showEditPopup = (v) => {
    this.setState({ editPopup: true, name: v.name, bcode: v.bcode, credit_limit: v.credit_limit, username: v.username, address1: v.address1, state: v.state, zip: v.zip, country: v.country, ccode: v.ccode, mobile: v.mobile, email: v.email, branch_id: v._id });
  };
                  ...
                  <tbody>
                  {
                      this.state.branches && this.state.branches.length > 0
                        ? this.state.branches.map(function(b) {
                          ...
                          return <tr key={b._id} ><td>{b.name}</td><td className="tac">{b.total_cashiers}</td><td className="tac">{b.credit_limit}</td>
